...
export const modal = b.createComponent<IData>({
    ...
    render(ctx: ICtx, me: b.IBobrilNode, oldMe: b.IBobrilCacheNode){
      ...
      me.style = { width: 700, height: 500, backgroundColor: "green", margin: "0px auto", top: 200, position: "relative", overflow: "hidden" },
      me.children = [ 
        header([ctx.data.modalName, b.style(button({title: "X", onClick: ()=>{ document.body.style.overflow = "auto"; if(ctx.data.onClose) ctx.data.onClose();}}), {position:"absolute", top: 5, right: 5,cursor:"pointer"})]),
        tablist(ctx.data, ctx, (tabName)=>{ ctx.selectedTab = tabName; b.invalidate(ctx); return true;}),
        content(ctx.content)]
    }
  })
...
function changeSelectedTab(name: string, tabList: any[]): any[]{
  tabList.forEach((e)=>{ e.tabName == name ? e.isSelected = true : e.isSelected = false});
  return tabList;
}
...
function tablist(modalCfg: IData, ctx: ICtx, onClick: (name: string)=>boolean): b.IBobrilNode{
  return b.styledDiv([ 
    ...
    ],{width:"100%", backgroundColor: "white", margin: 0, paddingLeft: 42, overflowY: "auto", height: 40})//, borderStyle:"none none solid"
}
...
function header(children: any): b.IBobrilNode{
  ...
  render(ctx: b.IBobrilCtx, me: b.IBobrilNode, oldMe: b.IBobrilCacheNode){
    me.style = {width:"100%", height: 50, backgroundColor: "white", margin: 0, fontSize: '2rem', paddingLeft: 50, paddingTop:10}
...
function content(modalContent: any, style?: any): b.IBobrilNode{
  return b.styledDiv( b.styledDiv(modalContent,{padding: 50}),
          {height: "calc(100% - 100px)", backgroundColor: "white", margin: 0, overflowY: "auto",
           borderTopStyle: "solid", borderTopWidth: 1, borderTopColor: "black"})
  }