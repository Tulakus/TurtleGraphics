...
export let create = b.createComponent<IData>({
    ...
    onDragStart(ctx: ICtx, dndCtx: b.IDndCtx): boolean{
        var dnd = ctx.data;
        ctx.startY = parseInt(document.getElementById('svgpane').style.top);
        ctx.startX = parseInt(document.getElementById('svgpane').style.left);
    ...
    render(ctx: ICtx, me: b.IBobrilNode, oldMe?: b.IBobrilCacheNode): void {
        ...
        me.style = { 
            width: ctx.origWidth, height: ctx.origHeight, backgroundColor: 'white', top: ctx.top, left: ctx.left, position:"relative", cursor: ctx.cursor||"auto",
    ...
    postInitDom(ctx: ICtx, me: b.IBobrilNode, element: HTMLElement) {
        ...
        let elmWidth = parseInt(element.style.width);
        let elmHeight = parseInt(element.style.height);
