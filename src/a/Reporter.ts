...
export class Reporter {
    ...
    createAnnotations(err: IError[]) {
        var instance = this.instance;//TODO
        var state = instance.state.lint;
        ...
        for (var line = 0; line < annotations.length; ++line) {
            ...
            for (var i = 0; i < anns.length; ++i) {
                ...
                if (state.hasGutter) tipLabel.appendChild(tip);
            ...
            if (state.hasGutter)
                cm.setGutterMarker(line, GUTTER_ID, this.makeMarker(tipLabel, maxSeverity, anns.length > 1,
                    state.options.tooltips));
            if (options.onUpdateLinting) options.onUpdateLinting(annotations, annotations, cm)
        }
